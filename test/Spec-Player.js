...
//      player.remove();
    ...
    it("should display sources when using the flash shim", function() {
      waitsFor(function() {
        return $('.mejs-sourcechooser-selector input[value$="media/echo-hereweare.mp4"]').length != 0
      }, "Timed Out", 10000);
        runs(function() {
            expect($('.mejs-sourcechooser-selector input[value$="media/echo-hereweare.mp4"]').length).toEqual(1);
            expect($('.mejs-sourcechooser-selector input[value$="media/other.mp4"]').length).toEqual(1);
        });
    });
});
